...
void merge_free_block(meta_d * header) {
...
void ff_free(void * ptr) {
  ...
  meta_d * prev_free_header = get_prev_header(blk_header);
  while (prev_free_header> free_first && prev_free_header->alloc != '0') {
    prev_free_header = get_prev_header(prev_free_header);
  }
...
int main(void) {
  ...
  // ff_free(h);
  // ff_free(b);
  // ff_free(a);//first block cannot be changed
